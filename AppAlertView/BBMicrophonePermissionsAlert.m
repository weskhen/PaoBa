//
//  BBMicrophonePermissionsAlert.m
//  PaoBa
//
...
#import "BBMicrophonePermissionsAlert.h"
#import "BBAlertController.h"

@interface BBMicrophonePermissionsAlert ()

@property (nonatomic, assign) BOOL  isAlertHaveShow;

@end

@implementation BBMicrophonePermissionsAlert
static BBMicrophonePermissionsAlert *instance = NULL;

+ (BBMicrophonePermissionsAlert *)sharedInstance{
    if (!instance) {
        instance = [BBMicrophonePermissionsAlert new];
    }
    return instance;
...
- (void)showAlertWithController:(UIViewController *)controller cancelClicked:(cancelHandler)cancelHandle setButtonClicked:(settingHandle)settingHandle
{
    ...
    BBAlertController *alertController = [BBAlertController alertControllerWithTitle:CCLocalizedString(@"popup_permission_required") message:CCLocalizedString(@"baba_access_microphonedesc") preferredStyle:BBAlertControllerStyleAlert];
