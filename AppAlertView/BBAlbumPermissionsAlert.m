...
#import "BBAlbumPermissionsAlert.h"
#import <AssetsLibrary/AssetsLibrary.h>
#import "BBAlertController.h"

@interface BBAlbumPermissionsAlert ()

@property (nonatomic, assign) BOOL  isAlertHaveShow;
@end

@implementation BBAlbumPermissionsAlert

static BBAlbumPermissionsAlert *instance = NULL;

+ (BBAlbumPermissionsAlert *)sharedInstance{
    if (!instance) {
        instance = [BBAlbumPermissionsAlert new];
    }
    return instance;
...
- (void)showAlertWithController:(UIViewController *)controller cancelClicked:(cancelHandler)cancelHandle setButtonClicked:(settingHandle)settingHandle
{
    ...
    BBAlertController *alertController = [BBAlertController alertControllerWithTitle:CCLocalizedString(@"popup_permission_required") message:CCLocalizedString(@"baba_access_photosdesc") preferredStyle:BBAlertControllerStyleAlert];
